...
/*
 *       DataFlash_MPNG.cpp - DataFlash log library for AT45DB161
 ...
 */
bool DataFlash_MPNG::_sem_take(uint8_t timeout)
{
...
void DataFlash_MPNG::ReadManufacturerID()
{
    if (!_sem_take(5))
        return;
...
bool DataFlash_MPNG::CardInserted()
{
    return true;
}
...
uint8_t DataFlash_MPNG::ReadStatusReg()
{
    uint8_t tmp;
...
// Assumes _spi_sem handled by caller.
void DataFlash_MPNG::WaitReady()
{
    while(!ReadStatus()) ;
}

void DataFlash_MPNG::PageToBuffer(uint8_t BufferNum, uint16_t PageAdr)
{
    if (!_sem_take(1))
        return;
...
void DataFlash_MPNG::BufferToPage (uint8_t BufferNum, uint16_t PageAdr, uint8_t wait)
{
    if (!_sem_take(1))
        return;
...
void DataFlash_MPNG::BlockWrite (uint8_t BufferNum, uint16_t IntPageAdr, 
...
bool DataFlash_MPNG::BlockRead(uint8_t BufferNum, uint16_t IntPageAdr, void *pBuffer, uint16_t size)
{
    if (!_sem_take(1))
        return false;
...
void DataFlash_MPNG::PageErase (uint16_t PageAdr)
{
    if (!_sem_take(1))
        return;
...
void DataFlash_MPNG::BlockErase (uint16_t BlockAdr)
{
    if (!_sem_take(1))
        return;
